...
# Privacy exclusion zone: remove GPS points within 1 km of the specified home area
# before any heatmap or route traces are generated. This prevents the published
# HTML from containing those coordinates.
HOME_COORDS = (41.895758, 140.6943699)
PRIVACY_RADIUS_KM = 1.0
...
def outside_privacy_zone(point: list[float]) -> bool:
    """Return True when a point is outside the configured privacy radius."""
    lat, lon = point
    home_lat, home_lon = HOME_COORDS
    return distance_km(lat, lon, home_lat, home_lon) > PRIVACY_RADIUS_KM
...
def filter_route_to_region(points: list[list[float]], bounds: tuple[float, float, float, float]) -> list[list[float]]:
    return [point for point in points if inside_bounds(point, bounds) and outside_privacy_zone(point)]
...
<div class="rivers-wrap">
  ...
  <div class="footer-note">Generated {generated}. Source data: local Strava archive FIT files filtered to {label}; GPS streams thinned for web delivery. A 1 km privacy exclusion zone is applied before map generation. Max embedded heatmap points: {max_heat_points:,}.</div>
</div>
</body>
</html>
"""
